<template>
  <div>
    <div class="d-flex flex-column flex-shrink-0 p-3 text-muted bg-light shadow-lg h-100">
    <img src="../../assets/bna-invoices.png" alt="logo" class="w-75 mx-auto mb-5 mt-4">
    <ul class="nav nav-pills flex-column mb-auto p-3">
      <li class="pb-3">
        <a href="#" class="nav-link text-primary">
          <i class="far fa-clock text-secondary"></i>
          Dashboard
        </a>
      </li>
      <li class="pb-3">
        <a href="goToClients()" class="nav-link text-primary">
          <i class="far fa-users text-secondary"></i>
          Clients
        </a>
      </li>
      <li class="pb-3">
        <a href="#" class="nav-link text-primary">
          <i class="far fa-analytics text-secondary"></i>
          Products
        </a>
      </li>
      <li class="pb-3">
        <a href="#" class="nav-link text-primary">
          <i class="far fa-file-invoice text-secondary"></i>
          Invoices
        </a>
      </li>
      <li class="pb-3">
        <a href="#" class="nav-link text-primary">
          <i class="fal fa-credit-card text-secondary"></i>
          Payments
        </a>
      </li>
      <li class="pb-3">
        <a href="#" class="nav-link text-primary">
          <i class="far fa-file-invoice text-secondary"></i>
          Credits
        </a>
      </li>
      <li class="pb-3">
        <a href="#" class="nav-link text-primary">
          <i class="far fa-chart-pie-alt text-secondary"></i>
          Tasks
        </a>
      </li>
      <li class="pb-3">
        <a href="#" class="nav-link text-primary">
          <i class="far fa-flag text-secondary"></i>
          Reports
        </a>
      </li>
      <li class="pb-3">
        <a href="#" class="nav-link text-primary">
          <i class="far fa-cog mr-5 text-secondary"></i>
          Settings
        </a>
      </li>
    </ul>
  </div>
  </div>
</template>

<script>
import ClientsTable from '../UI/clients/ClientsTable.vue'
export default {
  components: {
    ClientsTable
  },
  methods: {
    goToClients() {
      $router.push('./UI/clients/ClientsTable.vue')
    }
  }
}
</script>

<style scoped>
i {
  margin-right: 5px;
}
</style>